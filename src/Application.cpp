...
void Application::init(const std::string& resourceDirectory) {
	...
	initBox2DWorld();
	initEntities();
    initGUIProgram(resourceDirectory + "/shaders");
}
...
void Application::initGUIProgram(const std::string& resourceDirectory) {
...
// Set up vertex data (and buffer(s)) and attribute pointers
    ...
    unsigned char* toStart = stbi_load( "../resources/UI/tostart.png", &width, &height, &bpp, STBI_rgb_alpha);
    if(toStart == nullptr)
    throw(std::string("Failed to load texture"));

    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, toStart);
    ...
    guiProgram = make_shared<Program>();
    guiProgram->setVerbose(true);
    guiProgram->setShaderNames(resourceDirectory + "/tex_vert.glsl",
    ...
    guiProgram->addUniform("P");
    guiProgram->addUniform("V");
    guiProgram->addUniform("M");
    guiProgram->addUniform("offset");
    guiProgram->addUniform("w");
    guiProgram->addAttribute("vertPos");
    guiProgram->addAttribute("vertNor");
    guiProgram->addAttribute("vertTex");
    guiProgram->addUniform("Texture0");
}
